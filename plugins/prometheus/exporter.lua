...
-- Based on: https://github.com/Kong/kong/blob/3.9.1/kong/plugins/prometheus/exporter.lua
...
-- SPDX-License-Identifier: Apache-2.0
-- SPDX-SnippetCopyrightText: 2025 Deutsche Telekom AG
-- Reduced amount of latency buckets to reduce memory usage with enhanced sub-1000ms coverage
...
local function log(message, serialized)
  ...
  -- SPDX-SnippetEnd
      ...
      -- For stream, use 3-label version without consumer
      local labels_table_latency_stream = {service_name, route_name, workspace}
      
      local session_latency = serialized.latencies.session
      if session_latency and session_latency >= 0 then
        metrics.total_latency:observe(session_latency, labels_table_latency_stream)
      end
      
      local kong_proxy_latency = serialized.latencies.kong
      if kong_proxy_latency ~= nil and kong_proxy_latency >= 0 then
        metrics.kong_latency:observe(kong_proxy_latency, labels_table_latency_stream)
      end
    end
    -- SPDX-SnippetEnd
  end
