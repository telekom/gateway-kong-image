...
Stargate does not pin or bundle a Valkey/Redis version — Valkey is treated as
external infrastructure that should be set up separately. What the code requires is
only standard RESP commands (`INCR`, `EXPIRE`, `EXPIREAT`, `EXISTS`, `INCRBY`,
`EVAL`, `AUTH`, `SELECT`). No Valkey modules, no streams, no cluster-only features.

**Recommended version:** 8.x stable / 9.x stable.
...
Once Valkey is connected to the gateway one time, rate limiting works automatically
for every API — no manual, per-API gateway configuration is required.
